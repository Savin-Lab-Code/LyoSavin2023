...
@torch.no_grad()
def p_sample_loop(model, shape, n_steps, alphas, betas, one_minus_alphas_prod_sqrt, device, init_x=None):
    if init_x == None:
    ...
    for i in reversed(range(n_steps)):
        cur_x = p_sample(model, cur_x, i, n_steps, alphas,betas,one_minus_alphas_prod_sqrt, device)
        x_seq.append(cur_x)
    return x_seq

@torch.no_grad()
def p_sample_loop_new(model, shape, n_steps, device='cpu', init_x=None):
